	...
	<div id="home-experience" class="section section-first section-last">
		<div class="container">
			...
			<div
				class="timeline"
				v-bind:style="{
					backgroundImage: 'url(' + data.website.imgGrey + ')',
				}"
			>
				...
				>
					...
					<span v-if="language === 'Eng'">
						<p
							v-for="task in experience.tasks.eng"
							v-bind:key="task"
						>
							{{ task }}
						</p>
					...
					<span v-else-if="language === 'Fra'">
						<p
							v-for="task in experience.tasks.fra"
							v-bind:key="task"
						>
							{{ task }}
						</p>
					...
					<span v-else-if="language === 'Esp'">
						<p
							v-for="task in experience.tasks.esp"
							v-bind:key="task"
						>
							{{ task }}
						</p>
					...
					<span v-else-if="language === 'Chi'">
						<p
							v-for="task in experience.tasks.chi"
							v-bind:key="task"
						>
							{{ task }}
						</p>
...
export default {
	data() {
		return {
			data: Json,
			headshotpath: Json.website.imgHeadshot,
			language: 'Eng'
		}
	},
	...
	methods: {
		headshotOver() {
			this.headshotpath = Json.website.imgHeadshotThug
		},
		headshotLeave() {
			this.headshotpath = Json.website.imgHeadshot
		}
	}
}
</script>
